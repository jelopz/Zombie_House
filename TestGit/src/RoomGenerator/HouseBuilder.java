...
/**
 * RoomGenerator class for the main application. Creates a procedurally
 * generated map consisting of tiles stored in a 2D array of Tiles. The map is
 * completely space filled.
 * 
 * 
 * Given a width and height, HouseBuilder builds a map with these dimensions of
 * a large room. At each tile, a zombie has a chance to spawn there. Then,
 * HouseBuilder partitions the map into 4 quadrants. In these four quadrants we
 * are left with 4 large chunks of rooms. Per quadrant, we start placing
 * hallways (and doors for these hallways), splitting each quadrant up into
 * hallways and rooms randomly. Once done, we add the player spawn point in a
 * randomly selected quadrant and add the end point in the quadrant counter
 * clockwise to the player spawn. Then, from the player spawn, we make doorways
 * from quadrant to quadrant going clockwise.
 * 
 * Thus, a player must traverse through each quadrant to reach the end point.
 */
public class HouseBuilder
{
  ...
  /**
   * Given a tile, returns if that tile is denoted as the end of the map or not
   *
   * @param x
   *          the x
   * @param y
   ...
   *
   * @param quadrant
   *          The quadrant we are trying to find the top-left corner tile for.
